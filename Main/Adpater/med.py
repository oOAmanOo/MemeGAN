...
from adapters.methods.lora import LoRA as LoRALinear
from adapters.methods.prefix_tuning import PrefixTuning as PrefixTuningShim
from adapters.models.bert.mixin_bert import (
    BertModelAdaptersMixin,
    BertOutputAdaptersMixin,
    BertSelfOutputAdaptersMixin
)
from adapters.heads.model_mixin import ModelWithHeadsAdaptersMixin
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_co_attention):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        if is_co_attention == 1:
            mixed_query_layer = self.query(hidden_states)
            key_layer = self.transpose_for_scores(self.key(encoder_hidden_states))
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states))
            attention_mask = encoder_attention_mask
        elif is_co_attention == 2:
        ...
        if is_co_attention != 0 and self.save_attention:
...
class BertAttention(nn.Module):
    def __init__(self, config, is_co_attention=0):
        super().__init__()
        self.self = BertSelfAttention(config, is_co_attention)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        self.text_intermediate = BertIntermediate(config)
        self.image_intermediate = BertIntermediate(config)
        self.text_output = BertOutput(config)
        self.image_output = BertOutput(config)
